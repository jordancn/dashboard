...
export const isValid = (t: unknown): t is TimeIso => typeof t === 'string' && VALID_REGEX.test(t);

export function getDuration(timeSet: TimeSet): number {
  return DateFns.differenceInMinutes(parse(timeSet.endTime), parse(timeSet.startTime));
}
...
export function from(hours: number = 0, minutes: number = 0, seconds: number = 0): TimeIso {
...
export function timeIso(literals: TemplateStringsArray, ...placeholders: never[]) {
